...
public class SetStatusOperation extends DbOperation {

	private int assetStatus;
	private List<Asset> assets;
	private int[] oldStates;

	public SetStatusOperation(List<Asset> assets, int status) {
		super(Messages.getString("SetStatusOperation.set_status")); //$NON-NLS-1$
		this.assets = assets;
		this.assetStatus = status;
		oldStates = new int[assets.size()];
	}
	...
	public IStatus execute(IProgressMonitor aMonitor, IAdaptable info)
		...
		for (int i = 0; i < assets.size(); i++)
			oldStates[i] = assets.get(i).getStatus();
		setStatus(aMonitor);
		return close(info);
	}

	private void setStatus(IProgressMonitor monitor) {
		...
		fireApplyRules(assets, QueryField.STATUS);
		fireAssetsModified(new BagChange<>(null, assets, null, null), QueryField.STATUS);
	}
	...
	public IStatus undo(IProgressMonitor aMonitor, IAdaptable info)
		...
		List<Asset> toBeStored = new ArrayList<>(assets.size());
		int i = 0;
		for (Asset asset : assets) {
			if (asset.getFileState() != IVolumeManager.PEER) {
				asset.setStatus(oldStates[i]);
