...
public class PaintSurface {
	 ...
	 */
	public PaintSurface(Canvas paintCanvas, int w, int h, Text statusText,
			Color fillColor) {
	...
	public void setPaintSession(PaintSession paintSession) {
		...
		if (paintSession != null) {
			...
			setStatusMessage(PaintExample
					.getResourceString("session.Null.message")); //$NON-NLS-1$
		}
	}
	...
	public void scrollHorizontally(ScrollBar scrollBar) {
		...
		if (imageWidth > visibleWidth) {
			...
			if (oldOffset != newOffset) {
				...
				paintCanvas.scroll(Math.max(oldOffset - newOffset, 0), 0, Math
						.max(newOffset - oldOffset, 0), 0, visibleWidth,
						visibleHeight, false);
			}
		}
	}
	...
	public void setStatusCoord(Point coord) {
		statusCoordInfo = (coord != null) ? PaintExample.getResourceString(
				"status.Coord.format", new Object[] { Integer.valueOf(coord.x), //$NON-NLS-1$
						Integer.valueOf(coord.y) }) : ""; //$NON-NLS-1$
		updateStatus();
	}
	...
	public void setStatusCoordRange(Point a, Point b) {
		statusCoordInfo = PaintExample.getResourceString(
				"status.CoordRange.format", new Object[] { new Integer(a.x), //$NON-NLS-1$
