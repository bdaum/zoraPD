 ...
 * @author Berthold Daum, changed document identifier to StringField
 */
...
//    public Field[] createDescriptorFields(BufferedImage image) {
    public Field[] createDescriptorFields(BufferedImage image, String identifier) { //bd
        docsCreated = true;
...
//                Field[] fields = builder.createDescriptorFields(image);
            	Field[] fields = builder.createDescriptorFields(image, identifier); // bd
...
//            doc.add(new StoredField(DocumentBuilder.FIELD_NAME_IDENTIFIER, identifier));
        	doc.add(new StringField(DocumentBuilder.FIELD_NAME_IDENTIFIER, identifier, Field.Store.YES));  // bd
        if (builders.size() >= 1) {
            for (DocumentBuilder builder : builders) {
//                Field[] fields = builder.createDescriptorFields(image);
            	Field[] fields = builder.createDescriptorFields(image, identifier); //bd
