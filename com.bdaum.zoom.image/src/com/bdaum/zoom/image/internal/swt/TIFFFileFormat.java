 ...
 *     bd - allow for direct file access
 *******************************************************************************/
...
public final class TIFFFileFormat extends FileFormat {
	...
	ImageData[] loadFromByteStream() {
		...
		try {
			file = loader.filename != null ? new TIFFRandomFileAccess(
					loader.filename) : new TIFFRandomFileAccess(inputStream);
			...
			try {
				...
				int offset = isLittleEndian ? (header[4] & 0xFF)
						| ((header[5] & 0xFF) << 8)
						| ((header[6] & 0xFF) << 16)
						| ((header[7] & 0xFF) << 24) : (header[7] & 0xFF)
						| ((header[6] & 0xFF) << 8)
						| ((header[5] & 0xFF) << 16)
						| ((header[4] & 0xFF) << 24);
				file.seek(offset);
				TIFFDirectory directory = new TIFFDirectory(file,
						isLittleEndian, loader);
				ImageData image = directory.read();
				/*
				 * A baseline reader is only expected to read the first
				 * directory
				 */
	...
	void unloadIntoByteStream(ImageLoader loader) {
		/*
		 * We do not currently support writing multi-page tiff, so we use the
		 * first image data in the loader's array.
		 */
