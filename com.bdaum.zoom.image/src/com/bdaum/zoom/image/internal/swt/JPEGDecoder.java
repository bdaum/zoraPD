 ...
 *     Berthold Daum - added support for CMYK JPEGS
 *******************************************************************************/
...
public class JPEGDecoder {
	...
	static final int CMYK_RGB_CONVERT = 5; // bd
		...
		void start_pass(jpeg_decompress_struct cinfo, int pass_mode) {
			...
			case JBUF_PASS_THRU:
				if (cinfo.quantize_colors) {
					...
					// * allocate a strip buffer. Use the virtual-array buffer
					// as workspace.
					// */
					// if (post.buffer == null) {
					...
					/*
					 * For single-pass processing without color quantization, I
					 * have no work to do; just call the upsampler directly.
					 */
	...
	/* Allocate and fill in the sample_range_limit table */
	{
		...
		System.arraycopy(cinfo.sample_range_limit,
				cinfo.sample_range_limit_offset, table, offset
						+ (4 * (MAXJSAMPLE + 1) - CENTERJSAMPLE), CENTERJSAMPLE);
	}
	...
	static void jinit_color_deconverter(jpeg_decompress_struct cinfo) {
		...
		case JCS_CMYK:
			...
			if (cinfo.jpeg_color_space == JCS_YCCK) {
				...
				// cconvert.color_convert = NULL_CONVERT; // bd
				cconvert.color_convert = CMYK_RGB_CONVERT;
			} else
				error();
			// ERREXIT(cinfo, JERR_CONVERSION_NOTIMPL);
			break;
	...
	static void process_data_context_main(jpeg_decompress_struct cinfo,
		...
		if (!main.buffer_full) {
			...
			case DECOMPRESS_DATA:
				result = decompress_data(cinfo, main.xbuffer[main.whichptr],
			...
			case DECOMPRESS_ONEPASS:
				result = decompress_onepass(cinfo, main.xbuffer[main.whichptr],
						main.xbuffer_offset[main.whichptr]);
				break;
			default:
				result = 0;
	...
	static void sep_upsample(jpeg_decompress_struct cinfo,
			...
			for (ci = 0; ci < cinfo.num_components; ci++) {
				...
				case NOOP_UPSAMPLE:
					noop_upsample(cinfo, compptr, input_buf[ci], offset,
							upsample.color_buf, upsample.color_buf_offset, ci);
					break;
				case FULLSIZE_UPSAMPLE:
					fullsize_upsample(cinfo, compptr, input_buf[ci], offset,
							upsample.color_buf, upsample.color_buf_offset, ci);
					break;
				case H2V1_FANCY_UPSAMPLE:
					h2v1_fancy_upsample(cinfo, compptr, input_buf[ci], offset,
							upsample.color_buf, upsample.color_buf_offset, ci);
					break;
				case H2V1_UPSAMPLE:
				...
				case H2V2_FANCY_UPSAMPLE:
					h2v2_fancy_upsample(cinfo, compptr, input_buf[ci], offset,
							upsample.color_buf, upsample.color_buf_offset, ci);
					break;
				case H2V2_UPSAMPLE:
		...
		case NULL_CONVERT:
			null_convert(cinfo, upsample.color_buf, upsample.color_buf_offset,
					upsample.next_row_out, output_buf, out_row_ctr[0], num_rows);
			break;
		...
		case YCCK_CMYK_CONVERT:
			ycck_rgb_convert(cinfo, upsample.color_buf,
					upsample.color_buf_offset, upsample.next_row_out,
					output_buf, out_row_ctr[0], num_rows);
			break;
		case CMYK_RGB_CONVERT:
			cmyk_rgb_convert(cinfo, upsample.color_buf,
					upsample.color_buf_offset, upsample.next_row_out,
					output_buf, out_row_ctr[0], num_rows);
			break; // bd
		}
	...
	private static void ycck_rgb_convert(jpeg_decompress_struct cinfo,
		...
		while (--num_rows >= 0) {
			...
			for (col = 0; col < num_cols; col++) {
				y = (inptr0[col] & 0xFF);
				cb = (inptr1[col] & 0xFF) - 128;
				cr = (inptr2[col] & 0xFF) - 128;
				...
				ma = (y - (0.3441363f * cb) - (0.71413636f * cr));
				if (ma < 0f)
					ma = 0f;
				if (ma > 255f)
					ma = 255f;
				...
				outptr[outptr_offset + RGB_RED] = (byte) (255f - cy); // red
				outptr[outptr_offset + RGB_GREEN] = (byte) (255f - ma); // green
				outptr[outptr_offset + RGB_BLUE] = (byte) (255f - ye); // blue
	...
	static void cmyk_rgb_convert(
			jpeg_decompress_struct cinfo, // bd
			byte[][][] input_buf, int[] input_buf_offset, int input_row,
			byte[][] output_buf, int output_buf_offset, int num_rows) {
		int c, m, y;
		byte[] outptr;
		...
		while (--num_rows >= 0) {
			...
			for (col = 0; col < num_cols; col++) {
				...
				m = (inptr1[col] & 0xFF);
				y = (inptr2[col] & 0xFF);
				outptr[outptr_offset + RGB_RED] = (byte) (c); // red
				outptr[outptr_offset + RGB_GREEN] = (byte) (m); // green
				outptr[outptr_offset + RGB_BLUE] = (byte) (y); // blue
				outptr_offset += RGB_PIXELSIZE;
			}
		}
	}
	...
	/* Called once, when first SOS marker is reached */
	{
		...
		if (cinfo.data_precision != BITS_IN_JSAMPLE)
			error(" [data precision=" + cinfo.data_precision + "]"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	static int consume_markers(jpeg_decompress_struct cinfo) {
		...
		case JPEG_REACHED_SOS: /* Found SOS */
			if (inputctl.inheaders) { /* 1st SOS */
				...
				/*
				 * Note: start_input_pass must be called by jdmaster.c before
				 * any more input can be consumed. jdapimin.c is responsible for
				 * enforcing this sequencing.
				 */
	...
	static ImageData[] loadFromByteStream(InputStream inputStream,
		...
		int scanlinePad = 4; // bd
		int data_width = 0;
		switch (cinfo.out_color_space) {
		case JCS_CMYK: // bd
		case JCS_RGB:
			palette = new PaletteData(0xFF, 0xFF00, 0xFF0000);
			data_width = (((cinfo.output_width * 3 * 8 + 7) / 8) + (scanlinePad - 1))
		...
		case JCS_GRAYSCALE:
			...
			data_width = (((cinfo.output_width * 8 + 7) / 8) + (scanlinePad - 1))
					/ scanlinePad * scanlinePad; // bd
		...
		// byte[] data = new byte[row_stride * cinfo.output_height]; // bd
		byte[] data = new byte[data_width * cinfo.output_height]; // bd
		ImageData imageData = ImageData.internal_new(cinfo.output_width,
				cinfo.output_height, palette.isDirect ? 24 : 8, palette,
				scanlinePad, data, 0, null, null, -1, -1, SWT.IMAGE_JPEG, 0, 0,
				0, 0);
		if (cinfo.buffered_image) {
			boolean done;
			do {
				...
				loader.notifyListeners(new ImageLoaderEvent(loader,
						(ImageData) imageData.clone(), incrementCount,
						done = jpeg_input_complete(cinfo)));
			} while (!done);
		} else {
			while (cinfo.output_scanline < cinfo.output_height) {
				// int offset = row_stride * cinfo.output_scanline; // bd
				int offset = data_width * cinfo.output_scanline; // bd
				jpeg_read_scanlines(cinfo, buffer, 1);
				// System.arraycopy(buffer[0], 0, data, offset, row_stride);
				System.arraycopy(buffer[0], 0, data, offset, data_width);
			}
		}
