 ...
 *     bd - avoid memory blow-out on incomplete files
 *     bd - remove memory comsumption by using random accecc files
 *******************************************************************************/
...
final class TIFFRandomFileAccess {
	...
	byte[][] buffers;
	private RandomAccessFile raFile; // bd
...
public TIFFRandomFileAccess(String filename) throws FileNotFoundException { // bd
	raFile = new RandomAccessFile(filename, "r"); //$NON-NLS-1$
}
...
void seek(int pos) throws IOException {
	...
	if (current > next) {
		...
		while (n > 0) {
			...
			int len = Math.min(n, CHUNK_SIZE - offset);  // bd
			int cnt = inputStream.read(buffers[index], offset, len);
			if (cnt < len)  // bd
				throw new IOException(NLS.bind("{0} < {1}", cnt, len));  //$NON-NLS-1$ //bd
			n -= cnt;
			next += cnt;
			index++;
			offset = 0;
