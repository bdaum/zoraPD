...
public final class PNGFileFormat extends FileFormat {
	...
	/**
	 * Skip over signature data. This has already been verified in
	 * isFileFormat().
	 */
	...
	ImageData[] loadFromByteStream() {
		try {
			...
			imageData = ImageData.internal_new(width, height, headerChunk.getSwtBitsPerPixel(),
					new PaletteData(0, 0, 0), 4, data, 0, null, null, -1, -1, SWT.IMAGE_PNG, 0, 0, 0, 0);
	...
	/**
	 * SWT does not support 16-bit depths. If this image uses 16-bit depths,
	 * convert the data to an 8-bit depth.
	 */
	...
	/**
	 * SWT does not support greyscale as a color type. For plain grayscale, we
	 * create a palette. For Grayscale with Alpha, however, we need to convert
	 * the pixels to use RGB values. Note: This method assumes that the bit
	 * depth of the data has already been restricted to 8 or less.
	 */
	void setPixelData(byte[] data, ImageData imageData) {
		...
		case PngIhdrChunk.COLOR_TYPE_GRAYSCALE_WITH_ALPHA: {
			...
			/*
			 * If the image uses 16-bit depth, it is converted to an 8-bit depth
			 * image.
			 */
	 ...
	 * If the image uses an unsupported color type (either of the gray scale
	 * types) or bit depth (16), convert the data to an SWT-supported format.
	 * Then assign the data into the ImageData given.
	 */
	...
	/**
	 * Read the image data from the data stream. This must handle decoding the
	 * data, filtering, and interlacing.
	 */
	...
	void readPixelData(PngIdatChunk chunk, PngChunkReader chunkReader) throws IOException {
		...
		try {
			...
			/*
			 * InflaterInputStream does not consume all bytes in the stream when
			 * it is closed. This may leave unread IDAT chunks. The fix is to
			 * read all available bytes before closing it.
			 */
	...
	/**
	 * Answer the number of bytes in each row of the image data. Each PNG row is
	 * byte-aligned, so images with bit depths less than a byte may have unused
	 * bits at the end of each row. The value of these bits is undefined.
	 */
	int getBytesPerRow() {
	...
	/**
	 * Answer the number of bytes needed to represent a pixel. This value
	 * depends on the image's color type and bit depth. Note that this method
	 * rounds up if an image's pixel size isn't byte-aligned.
	 */
	...
	/**
	 * 1. Read one of the seven frames of interlaced data. 2. Update the
	 * imageData. 3. Notify the image loader's listeners of the frame load.
	 */
	void readInterlaceFrame(InputStream inputStream, int rowInterval, int columnInterval, int startRow, int startColumn,
	...
	/**
	 * Fire an event to let listeners know that an interlaced frame has been
	 * loaded. finalFrame should be true if the image has finished loading,
	 * false if there are more frames to come.
	 */
	...
	/**
	 * SWT does not support 16-bit depth color formats. Convert the 16-bit data
	 * to 8-bit data. The correct way to do this is to multiply each 16 bit
	 * value by the value: (2^8 - 1) / (2^16 - 1). The fast way to do this is
	 * just to drop the low byte of the 16-bit value.
	 */
	static void compress16BitDepthTo8BitDepth(byte[] source, int sourceOffset, byte[] destination,
			int destinationOffset, int numberOfValues) {
		// double multiplier = (Compatibility.pow2(8) - 1) /
		// (Compatibility.pow2(16) - 1);
	...
	/**
	 * PNG supports four filtering types. These types are applied per row of
	 * image data. This method unfilters the given row based on the filterType.
	 */
