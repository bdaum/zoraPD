...
public class IniReader {
	...
	public IniReader(String pathname, boolean caseSensitive) throws FileNotFoundException, IOException {
		initialize(new FileReader(pathname), caseSensitive);
	}

	public IniReader(File metafile, boolean caseSensitive) throws FileNotFoundException, IOException {
		initialize(new FileReader(metafile), caseSensitive);
	}
	
	public IniReader(InputStream in, boolean caseSensitive) throws FileNotFoundException, IOException {
		initialize(new InputStreamReader(in), caseSensitive);
	}
		...
		try (BufferedReader r = new BufferedReader(reader)) {
			while ((line = r.readLine()) != null) {
				...
				if (line.startsWith("[")) { //$NON-NLS-1$
					...
					if (!cs)
						key = key.toLowerCase();
					map.put(key, line.substring(index + 1).trim());
				}
			}
		}
	}
	...
	public String getPropertyString(String section, String key, String defaultValue) {
		Map<String, String> map = caseSensitive ? sections.get(section) : sections.get(section.toLowerCase());
		if (map != null) {
			String value = caseSensitive ? map.get(key) : map.get(key.toLowerCase());
			if (value != null)
				return value;
		}
		return defaultValue;
	...
	public float[] getFloatArray(String section, String key, float[] dflt) { // bd
		String s = getPropertyString(section, key, null);
		if (s != null) {
			List<String> tokens = CommonUtilities.fromStringList(s, ";"); //$NON-NLS-1$
	...
	public String[] getStringArray(String section, String key) { // bd
		List<String> tokens = CommonUtilities.fromStringList(getPropertyString(section, key, null), ";"); //$NON-NLS-1$
	...
	public Collection<String> listProperties(String section) {
		Map<String, String> map = caseSensitive ? sections.get(section) : sections.get(section.toLowerCase());
		if (map != null)
			return map.keySet();
		return null;
