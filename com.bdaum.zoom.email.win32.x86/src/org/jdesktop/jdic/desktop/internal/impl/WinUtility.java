...
public class WinUtility {
	 ...
	 * 
	 * @param file
	 *            the given File object, which contains the absolute path of a
	 *            file.
	...
	private static String getFileExtension(File file) {
		...
		if (trimFileStr == null || trimFileStr.length() == 0) {
			return null;
		}

		int strIndex = trimFileStr.lastIndexOf(File.separator);
		String filePart = trimFileStr.substring(strIndex + 1, trimFileStr
				.length());
	 ...
	 * <p>
	 * For example: suppose the given file is C:\test\test.txt, and the given
	 * verb is "print". The associated command string is retrieved as
	 ...
	 * @param verb
	 *            the given verb.
	 * @return the command string in the Registry associated with the given file
	 *         type and verb.
	 */
	...
	/**
	 * Gets the command string associated with the given URL's protocol and verb
	 * in the Registry.
	 * <p>
	 * For example: suppose the given URL is http://www.foo.com, and the given
	 * verb is "open". The associated command string is retrieved as
	 ...
	 * @param verb
	 *            the given verb.
	 * @return the command string in the Registry associated with the given URL
	 *         protocol and verb.
	 */
	public static String getVerbCommand(URL url, String verb) {
		String protocolType = url.getProtocol().trim();
		// For now, this api uses browser to open a file. So replace file
		// protocol
		// to http protocol.
	...
	static String getDefaultMailer() {
		String defaultMailer = WinAPIWrapper
				.WinRegQueryValueEx(WinAPIWrapper.HKEY_LOCAL_MACHINE,
	 ...
	 * 
	 * @param defMailer
	 *            Name of the system default mailer This mailer is either
	 *            "Mozilla" or "Mozilla Thunderbird"
	...
	static String getMozMailerLocation(String defMailer) {
		String mailerPath = WinAPIWrapper.WinRegQueryValueEx(
				WinAPIWrapper.HKEY_LOCAL_MACHINE, "SOFTWARE\\Clients\\Mail\\"
						+ defMailer + "\\shell\\open\\command", "");
	...
	static boolean isMapiSupported() {
		String regMapi = WinAPIWrapper.WinRegQueryValueEx(
				WinAPIWrapper.HKEY_LOCAL_MACHINE,
				"SOFTWARE\\Microsoft\\Windows Messaging Subsystem", "MAPI");
		if (regMapi != null) {
			if (regMapi.equals("1")) {
				return true;
			}
