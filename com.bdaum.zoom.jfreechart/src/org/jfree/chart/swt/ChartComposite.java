 ...
 * 08-Aug-2017 : Added Save as PDF (bd)
 */
 ...
 */
public class ChartComposite extends Composite implements ChartChangeListener, ChartProgressListener, PaintListener,
	...
	public static final String PROPERTIES_COMMAND = "PROPERTIES"; //$NON-NLS-1$

	/** Save action command. */
	public static final String SAVE_COMMAND = "SAVE"; //$NON-NLS-1$

	/** Print action command. */
	public static final String PRINT_COMMAND = "PRINT"; //$NON-NLS-1$
	...
	protected static ResourceBundle localizationResources = ResourceBundleWrapper
			.getBundle("org.jfree.chart.LocalizationBundle"); //$NON-NLS-1$
	 ...
	 *            the chart.
	 */
	public ChartComposite(Composite comp, int style, JFreeChart chart, boolean properties, boolean save, boolean print,
	...
	public ChartComposite(Composite comp, int style, JFreeChart jfreechart, int width, int height, int minimumDrawW,
			int minimumDrawH, int maximumDrawW, int maximumDrawH, boolean usingBuffer, boolean properties, boolean save,
		...
		this.canvas.addDisposeListener(new DisposeListener() {
			public void widgetDisposed(DisposeEvent e) {
				...
				img = (org.eclipse.swt.graphics.Image) canvas.getData("double-buffer-image"); //$NON-NLS-1$
				if (img != null) {
					img.dispose();
				}
			}
	 ...
	 *
	 * @param anchor
	 *            the anchor point ({@code null} permitted).
	 */
	 ...
	 *
	 * @param flag
	 *            {@code true} enables zooming if possible.
	 */
	...
	/**
	 * Returns {@code true} if file extensions should be enforced, and {@code false}
	 * otherwise.
	 *
	 * @return The flag.
	 */
	...
	public void doSaveAs() throws IOException {
		...
		String[] extensions = { "*.png", "*.pdf" }; //$NON-NLS-1$ //$NON-NLS-2$
		String[] names = { "Portable Network Graphics (*.png)", "Portable Document Format (*.pdf)" }; //$NON-NLS-1$ //$NON-NLS-2$
		...
		if (filename != null) {
			...
			if (file.exists()) {
				switch (filterIndex) {
				case 0:
					if (!MessageDialog.openQuestion(getShell(), Constants.APPLICATION_NAME,
							Messages.getString("ChartComposite.exists_overwrite"))) //$NON-NLS-1$
						return;
					break;
				case 1:
					MessageDialog dialog = new MessageDialog(getShell(), Constants.APPLICATION_NAME, null,
							Messages.getString("ChartComposite.already_exists"), MessageDialog.QUESTION, //$NON-NLS-1$
							new String[] { Messages.getString("ChartComposite.overwrite"), //$NON-NLS-1$
									Messages.getString("ChartComposite.append"), //$NON-NLS-1$
									Messages.getString("ChartComposite.cancel") }, //$NON-NLS-1$
							1);
					int ret = dialog.open();
					switch (ret) {
					case 0:
			...
			case 1:
				try {
					ChartPdf.saveChartAsPDF(new File(filename), this.chart, this.canvas.getSize().x,
							this.canvas.getSize().y);
	 ...
	 *
	 * @param flag
	 *            {@code true} enables trace lines for the mouse pointer on the
	 *            horizontal axis.
	 */
	...
	public void createChartPrintJob() {
		new ChartPrintJob("PlotPrint").print(this); //$NON-NLS-1$
	}
	...
	protected Menu createPopupMenu(boolean properties, boolean save, boolean print, boolean zoom) {
		...
		if (properties) {
			...
			propertiesItem.setText(localizationResources.getString("Properties...")); //$NON-NLS-1$
		...
		if (save) {
			...
			saveItem.setText(localizationResources.getString("Save_as...")); //$NON-NLS-1$
		...
		if (print) {
			...
			printItem.setText(localizationResources.getString("Print...")); //$NON-NLS-1$
		...
		if (zoom) {
			...
			zoomInMenuItem.setText(localizationResources.getString("Zoom_In")); //$NON-NLS-1$
			zoomInMenuItem.setMenu(zoomInMenu);

			this.zoomInBothMenuItem = new MenuItem(zoomInMenu, SWT.PUSH);
			this.zoomInBothMenuItem.setText(localizationResources.getString("All_Axes")); //$NON-NLS-1$
			...
			this.zoomInDomainMenuItem.setText(localizationResources.getString("Domain_Axis")); //$NON-NLS-1$
			...
			this.zoomInRangeMenuItem.setText(localizationResources.getString("Range_Axis")); //$NON-NLS-1$
			...
			zoomOutMenuItem.setText(localizationResources.getString("Zoom_Out")); //$NON-NLS-1$
			zoomOutMenuItem.setMenu(zoomOutMenu);

			this.zoomOutBothMenuItem = new MenuItem(zoomOutMenu, SWT.PUSH);
			this.zoomOutBothMenuItem.setText(localizationResources.getString("All_Axes")); //$NON-NLS-1$
			...
			this.zoomOutDomainMenuItem.setText(localizationResources.getString("Domain_Axis")); //$NON-NLS-1$
			...
			this.zoomOutRangeMenuItem.setText(localizationResources.getString("Range_Axis")); //$NON-NLS-1$
			...
			autoRangeMenuItem.setText(localizationResources.getString("Auto_Range")); //$NON-NLS-1$
			autoRangeMenuItem.setMenu(autoRangeMenu);

			this.zoomResetBothMenuItem = new MenuItem(autoRangeMenu, SWT.PUSH);
			this.zoomResetBothMenuItem.setText(localizationResources.getString("All_Axes")); //$NON-NLS-1$
			...
			this.zoomResetDomainMenuItem.setText(localizationResources.getString("Domain_Axis")); //$NON-NLS-1$
			...
			this.zoomResetRangeMenuItem.setText(localizationResources.getString("Range_Axis")); //$NON-NLS-1$
	...
	public void widgetSelected(SelectionEvent e) {
		...
		if (command.equals(PROPERTIES_COMMAND)) {
			...
			try {
				...
				MessageDialog.openError(getShell(), Constants.APPLICATION_NAME,
						Messages.getString("ChartComposite.error_saving")); //$NON-NLS-1$
			}
	...
	public void paintControl(PaintEvent e) {
		...
		if (this.useBuffer) {
			// SwtGraphics2D sg2 = new SwtGraphics2D(e.gc);
			this.chartBuffer = (org.eclipse.swt.graphics.Image) this.canvas.getData("double-buffer-image"); //$NON-NLS-1$
			...
			if (this.refreshBuffer) {
				...
				this.canvas.setData("double-buffer-image", this.chartBuffer); //$NON-NLS-1$
				sg2d.dispose();
				gci.dispose();
				this.refreshBuffer = false;
			}
