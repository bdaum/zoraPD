 ...
 *                   Berthold Daum Graduated fills for rectangles)
 *
 * Changes
 * -------
 ...
 * 
 * 05-Feb-2017 : graduated fills for rectangles (bd)
 *
 */
...
public class SWTGraphics2D extends Graphics2D {
	...
	private Paint theRealPaint; // bd - remember paint for graduated fills
	 ...
	 *
	 * @param hintKey
	 *            the hint key (<code>null</code> permitted).
	 *
	 ...
	 * @param hintValue
	 *            the value (must be compatible with the specified key).
	 *
	 * @throws IllegalArgumentException
	 *             if <code>hintValue</code> is not compatible with the
	 *             <code>hintKey</code>.
	 *
	 ...
	 *
	 * @param hints
	 *            the hints (<code>null</code> not permitted).
	 *
	 ...
	 *
	 * @param paint
	 *            the paint (<code>null</code> permitted, ignored).
	 *
	 * @see #getPaint()
	 * @see #setColor(Color)
	 */
	public void setPaint(Paint paint) {
		this.theRealPaint = paint; // bd - remember paint for graduated fills
		if (paint == null) {
	 ...
	 *
	 * @param color
	 *            the color (<code>null</code> permitted but ignored).
	 *
	 * @see #getColor()
	 */
	 ...
	 *
	 * @param comp
	 *            the composite (<code>null</code> not permitted).
	 */
	...
	public Stroke getStroke() {
		return new BasicStroke(this.gc.getLineWidth(), toAwtLineCap(this.gc.getLineCap()),
	 ...
	 * @param x2
	 *            the x-coordinate for the ending point.
	 * @param y2
	 *            the y-coordinate for the ending point.
	 *
	 * @see #draw(Shape)
	 */
	 ...
	 *            the frame height.
	 * @param arcStart
	 *            the arc starting point, in degrees.
	 * @param arcAngle
	 ...
	 *            the frame height.
	 * @param arcWidth
	 *            the width of the arc defining the roundedness of the
	 *            rectangle's corners.
	 * @param arcHeight
	 *            the height of the arc defining the roundedness of the
	 *            rectangle's corners.
	 *
	...
	public void fill(Shape shape) {
		if (shape instanceof Rectangle2D && theRealPaint instanceof GradientPaint) { // bd
																				// -
																				// use
																				// fillRect
																				// method
			...
			fillRect((int) (rect.getX() + 0.5d), (int) (rect.getY() + 0.5d), (int) (rect.getWidth() + 0.5d),
					(int) (rect.getHeight() + 0.5d));
		} else {
			Path path = toSwtPath(shape);
	...
	public void fillRect(int x, int y, int width, int height) {
		this.switchColors();
		// bd - allow gradient fills for rectangles (only horizontal or
		// vertical)
		if (theRealPaint instanceof GradientPaint) {
			GradientPaint gradientPaint = (GradientPaint) theRealPaint;
			Point2D point1 = gradientPaint.getPoint1();
			Point2D point2 = gradientPaint.getPoint2();
			double hDiff = point1.getX() - point2.getX();
			double vDiff = point1.getY() - point2.getY();
			if (vDiff == 0 || hDiff == 0) { // only horizontal or vertical gradients supported
				org.eclipse.swt.graphics.Color savedBg = this.gc.getBackground();
				org.eclipse.swt.graphics.Color savedFg = this.gc.getForeground();
				org.eclipse.swt.graphics.Color swtColor1 = getSwtColorFromPool(gradientPaint.getColor1());
				org.eclipse.swt.graphics.Color swtColor2 = getSwtColorFromPool(gradientPaint.getColor2());
				boolean vertical = vDiff != 0;
				boolean reverse = (vertical ? vDiff : hDiff) < 0;
				this.gc.setBackground(reverse ? swtColor2 : swtColor1);
				this.gc.setForeground(reverse ? swtColor1 : swtColor2);
				this.gc.fillGradientRectangle(x, y, width, height, vertical);
				this.gc.setForeground(savedFg);
				this.gc.setBackground(savedBg);
			} else
	 ...
	 * 
	 * @param g
	 *            the glyph vector (<code>null</code> not permitted).
	 * @param x
	 *            the x-coordinate.
	 * @param y
	 *            the y-coordinate.
	 ...
	 *            the y-coordinate.
	 * @param width
	 *            the width for the rendered image.
	 * @param height
	 *            the height for the rendered image.
	 * @param observer
	 *            ignored here.
	 *
	 * @return <code>true</code> if the image has been drawn.
	...
	private org.eclipse.swt.graphics.Color getSwtColorFromPool(Color awtColor) {
		...
		if (swtColor == null) {
			...
			// see comment above
			// this.colorsPool.put(Integer.valueOf(awtColor.getRGB()),
			// swtColor);
