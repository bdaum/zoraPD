...
public class PaintSurface implements Listener {
	 ...
	 * <p>
	 * paintCanvas must have SWT.NO_REDRAW_RESIZE and SWT.NO_BACKGROUND styles, and
	 * may have SWT.V_SCROLL and/or SWT.H_SCROLL.
	 * </p>
	 *
	 * @param paintCanvas
	 ...
	 */
	public PaintSurface(Canvas paintCanvas, int w, int h, Text statusText, Color fillColor) {
	...
	public void setPaintSession(PaintSession paintSession) {
		...
		if (paintSession != null) {
			...
			setStatusAction(PaintExample.getResourceString("tool.Null.label")); //$NON-NLS-1$
			setStatusMessage(PaintExample.getResourceString("session.Null.message")); //$NON-NLS-1$
		}
	}
	 ...
	 * <p>
	 * Redraws any rubberband objects on the screen as permanent objects then clears
	 * the selection.
	 * </p>
	 */
	 ...
	 * <p>
	 * Decrements by one the rubberband "hide" nesting count. The rubberband is only
	 * made visible when showRubberband() has been called once for each previous
	 * hideRubberband(). It is not permitted to call showRubberband() if the rubber
	 * band is not presently hidden.
	 * </p>
	 */
	public void showRubberband() {
		if (rubberbandHiddenNestingCount <= 0)
			throw new IllegalStateException("rubberbandHiddenNestingCount > 0"); //$NON-NLS-1$
		if (--rubberbandHiddenNestingCount == 0) {
			rubberband.draw(displayFDC);
		}
	}
	...
	public void clearStatus() {
		statusActionInfo = ""; //$NON-NLS-1$
		statusMessageInfo = ""; //$NON-NLS-1$
		statusCoordInfo = ""; //$NON-NLS-1$
		updateStatus();
	}
	...
	public void setStatusAction(String action) {
		statusActionInfo = (action != null) ? action : ""; //$NON-NLS-1$
		updateStatus();
	}
	...
	public void setStatusMessage(String message) {
		statusMessageInfo = (message != null) ? message : ""; //$NON-NLS-1$
		updateStatus();
	}
	...
	public void setStatusCoord(Point coord) {
		statusCoordInfo = (coord != null)
				? PaintExample.getResourceString("status.Coord.format", new Object[] { Integer.valueOf(coord.x), //$NON-NLS-1$
						Integer.valueOf(coord.y) })
				: ""; //$NON-NLS-1$
		updateStatus();
	}
	...
	public void setStatusCoordRange(Point a, Point b) {
		statusCoordInfo = PaintExample.getResourceString("status.CoordRange.format", new Object[] { new Integer(a.x), //$NON-NLS-1$
				Integer.valueOf(a.y), Integer.valueOf(b.x), Integer.valueOf(b.y) });
		updateStatus();
	}
	...
	private void updateStatus() {
		if (statusText != null)
			statusText.setText(PaintExample.getResourceString("status.Bar.format", //$NON-NLS-1$
