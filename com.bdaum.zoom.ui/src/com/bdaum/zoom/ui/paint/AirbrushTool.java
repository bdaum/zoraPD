...
public class AirbrushTool extends ContinuousPaintSession implements PaintTool {
	 ...
	 * @param paintExample
	 */
	public AirbrushTool(ToolSettings toolSettings, PaintExample paintExample) {
		super(paintExample);
		random = new Random();
	...
	public String getDisplayName() {
		return PaintExample.getResourceString("tool.Airbrush.label"); //$NON-NLS-1$
	}
	...
	protected void render(Point point) {
		// Draws a bunch (cachedNumPoints) of random pixels within a specified circle
		// (cachedRadiusSquared).
		...
		for (int i = 0; i < cachedNumPoints; ++i) {
			...
			cfig.add(new PointFigure(settings.commonForegroundColor, settings.airbrushOpacity, point.x + randX, point.y + randY));
		}
		PaintSurface ps = getPaintSurface();
		ps.drawFigure(cfig);
		cfig.addDamagedRegion(ps.getDisplayFDC(), damaged);
