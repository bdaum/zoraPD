...
public abstract class UiDaemon extends Daemon {
	 ...
	 */
	public UiDaemon(String name, int minDelay, int maxDelay, int percentCpu) {
		super(name, maxDelay);
	...
	protected long computeDelay() {
		return Math.max(
				...
				Math.min(
						maxDelay,
						(System.nanoTime() - timeLastJobStarted)
								* factor
								* (Ui.isWorkbenchActive() ? 1
										: 4) / 1000000));
	}
