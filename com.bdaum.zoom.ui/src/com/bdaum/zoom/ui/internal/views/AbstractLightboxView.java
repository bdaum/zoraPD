...
public abstract class AbstractLightboxView extends AbstractGalleryView
	...
	protected final class GalleryPaintListener implements Listener {
		...
		public void handleEvent(Event e) {
			...
			if (showRotateButtons) {
				...
				bounds1.width = w + 10;
				bounds1.height = h + 10;
				Image rotate90icon = ((rot == R90) ? Icons.rotate90f : (isSelected) ? Icons.rotate90 : Icons.rotate90d)
						.getImage();
			...
			if (showVoicenoteButton && voiceFileURI != null && voiceFileURI.length() > 0) {
			...
			// Rating
			if (!PreferenceConstants.SHOWRATING_NO.equals(showRating)) {
	...
	protected static class Hotspots {
		...
		public String getTooltip(int x, int y, Asset asset) {
			...
			if (isVoicenotes(x, y))
				return Messages.getString("AbstractLightboxView.play_voicenote"); //$NON-NLS-1$
			return null;
		}
					...
					if (asset != null) {
						...
						if (hotSpots != null) {
							...
							if (hotSpots.isRating(e.x, e.y)) {
								...
								ZoomActionFactory.rate(Collections.singletonList(asset), AbstractLightboxView.this, dialog.open());
								break;
							}
		...
		preferenceStore.addPropertyChangeListener(new IPropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent event) {
				...
				if (PreferenceConstants.SHOWROTATEBUTTONS.equals(property))
					...
					showVoicenoteButton = preferenceStore.getBoolean(PreferenceConstants.SHOWVOICENOTE);
				else if (PreferenceConstants.SHOWEXPANDCOLLAPSE.equals(property))
					showExpandCollapseButton = preferenceStore.getBoolean(PreferenceConstants.SHOWEXPANDCOLLAPSE);
