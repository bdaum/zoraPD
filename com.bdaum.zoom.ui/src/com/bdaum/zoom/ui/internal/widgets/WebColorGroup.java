...
public class WebColorGroup {
	...
	public void setRGB(Rgb_type rgb) {
		...
		GC gc = new GC(image);
		Color c = new Color(button.getShell().getDisplay(), rgb.getR(),
				rgb.getG(), rgb.getB());
		gc.setBackground(c);
	...
	public Rgb_type fillValues(IDialogSettings dialogSettings, String key,
			int r, int g, int b) {
		...
		try {
			...
			if (colorSpec != null) {
				java.awt.Color color = new java.awt.Color(
						Integer.parseInt(colorSpec));
				Rgb_typeImpl rgb1 = new Rgb_typeImpl(color.getRed(),
	...
	public void saveSettings(IDialogSettings dialogSettings, String key) {
		if (rgb != null)
			dialogSettings.put(key, new java.awt.Color(rgb.getR(), rgb.getG(),
					rgb.getB()).getRGB());
	}
