...
public class Base64 {
	...
	private static int decode4to3(byte[] source, int srcOffset, byte[] destination, int destOffset, int options) {
		...
		if (srcOffset < 0 || srcOffset + 3 >= source.length) {
			throw new IllegalArgumentException(
					String.format("Source array with length %d cannot have offset of %d and still process four bytes.", //$NON-NLS-1$
	...
	public static byte[] decode(byte[] source, int off, int len, int options) throws java.io.IOException {
		...
		if (off < 0 || off + len > source.length) {
			throw new IllegalArgumentException(
					String.format("Source array with length %d cannot have offset of %d and process %d bytes.", //$NON-NLS-1$
		...
		for (i = off; i < off + len; i++) { // Loop through source
			...
			if (sbiDecode >= WHITE_SPACE_ENC) {
				...
				throw new java.io.IOException(
						String.format("Bad Base64 input character '%c' in array position %d", source[i], i)); //$NON-NLS-1$
			} // end else:
	...
	public static byte[] decode(String s, int options) throws java.io.IOException {
		...
		if ((bytes != null) && (bytes.length >= 4) && (!dontGunzip)) {
			...
			if (java.util.zip.GZIPInputStream.GZIP_MAGIC == head) {
				...
				} catch (java.io.IOException e) { // end try bais
					e.printStackTrace();
	...
	public static byte[] decodeFromFile(String filename) throws java.io.IOException {
		...
		try {
			...
			if (file.length() > Integer.MAX_VALUE) {
				throw new java.io.IOException(
						"File is too big for this convenience method (" + file.length() + " bytes)."); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public static void encodeFileToFile(String infile, String outfile) throws java.io.IOException {
		...
		try {
			...
			out.write(encoded.getBytes("US-ASCII")); // Strict, //$NON-NLS-1$
														// 7-bit output.
		} // end try
		catch (java.io.IOException e) {
			throw e; // Catch and release to execute finally{}
