 ...
 * having to issue the PORT command themselves and dealing with platform having
 * to issue the PORT (or EPRT) command themselves and dealing with platform
 * dependent issues.
 * <p>
 ...
 * f.connect(server);
 * f.login(username, password);
 * FTPListParseEngine engine = f.initiateListParsing(&quot;com.whatever.YourOwnParser&quot;,
 * 		directory);
 * 
...
public class FTPClient extends FTP implements Configurable {
	...
	protected Socket _openDataConnection_(int command, String arg)
		...
		if (__dataConnectionMode == ACTIVE_LOCAL_DATA_CONNECTION_MODE) {
			...
			if (!FTPReply.isPositiveCompletion(eprt(getLocalAddress(), server
					.getLocalPort()))) {
				// server.close();
				// return null;
...
//				return null;
				            ...
				            if (epsv() == FTPReply.ENTERING_EPSV_MODE) {
				                __parseExtendedPassiveModeReply(_replyLines.get(_replyLines.size() - 1));
				            }
				            else {
...
//			-            __parsePassiveModeReply((String)_replyLines.elementAt(0)); //TODO check this
			                if (pasv() != FTPReply.ENTERING_PASSIVE_MODE)
			                    return null;
			                __parsePassiveModeReply(_replyLines.get(_replyLines.size() - 1));
			            }
	...
	boolean reinitialize() throws IOException {
		...
		if (FTPReply.isPositiveCompletion(_replyCode)
				|| (FTPReply.isPositivePreliminary(_replyCode) && FTPReply
		...
		if (__entryParser == null) {
			if (null != parserKey) {
				...
				if (null != __configuration) {
					__entryParser = __parserFactory
							.createFileEntryParser(__configuration);
				} else {
	...
	private FTPListParseEngine initiateListParsing(FTPFileEntryParser parser,
		...
		try {
			engine
					.readServerList(socket.getInputStream(),
							getControlEncoding());
		} finally {
			socket.close();
		}
