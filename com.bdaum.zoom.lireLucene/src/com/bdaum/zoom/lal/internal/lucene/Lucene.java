...
public class Lucene implements ILuceneService {
	...
	public final class SearchHits implements ISearchHits {
		...
		public float score(int position) {
			return (float) hits.score(position);
		}
		...
		public String getAssetId(int position) throws IOException {
			int docId = hits.documentID(position);
			return getFieldValue(reader.document(docId), DocumentBuilder.FIELD_NAME_IDENTIFIER);
		}
	...
	public IDocumentIterator getDocumentIterator(Object readerToken, String assetId) throws IOException {
		IndexReader indexReader = readerMap.get(readerToken);
		if (indexReader == null)
			return null;
		return new DocumentIterator(indexReader, new Term(DocumentBuilder.FIELD_NAME_IDENTIFIER, assetId));
	}
	...
	public void releaseIndexWriter(File indexPath) throws CorruptIndexException, IOException {
	...
	public ISearchHits search(Object readerToken, int docId, int method, int maxResults) throws IOException {
		IndexReader indexReader = readerMap.get(readerToken);
		if (indexReader != null) {
			ImageSearchHits hits = LireActivator.getDefault().search(indexReader, docId, method, maxResults);
			if (hits != null)
				return new SearchHits(indexReader, hits);
		}
		return null;
	}

	public ISearchHits search(Object readerToken, BufferedImage image, int method, int maxResults) throws IOException {
		IndexReader indexReader = readerMap.get(readerToken);
		if (indexReader != null) {
			ImageSearchHits hits = LireActivator.getDefault().search(indexReader, image, method, maxResults);
			if (hits != null)
				return new SearchHits(indexReader, hits);
		}
		return null;
	...
	public ISearchHits search(Object readerToken, String queryString, int maxResults)
		...
		if (indexReader != null) {
			...
			return new ISearchHits() {
				...
				public String getAssetId(int position) throws IOException {
					return getFieldValue(indexReader.document(hits.scoreDocs[position].doc),
