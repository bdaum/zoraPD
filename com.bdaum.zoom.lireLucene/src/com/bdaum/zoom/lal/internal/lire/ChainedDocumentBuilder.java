...
/**
 * Chaining of DocumentBuilder. If you need several different feature, create a
 * ChainedDocumentBuilder and add different DocumentBuilder instances with
...
public class ChainedDocumentBuilder extends AbstractLocalDocumentBuilder {
	...
	/**
	 * Adds DocumentBuilder instances to the ChainedDocumentBuilder. Note that
	 * after Document instances have been created with
	 * {@link ChainedDocumentBuilder#createDocument(java.awt.image.BufferedImage, String)}
	 * no new DocumentBuilder instances can be added.
	 * 
	 * @param builder
	...
	public void addBuilder(DocumentBuilder builder) {
		if (docsCreated)
			throw new UnsupportedOperationException("Cannot modify chained builder after documents have been created!"); //$NON-NLS-1$
	...
	public Field[] createDescriptorFields(BufferedImage image, String identifier) {
		...
		for (DocumentBuilder builder : builders)
			Collections.addAll(resultList, builder.createDescriptorFields(image, identifier));
	...
	public Document createDocument(BufferedImage image, String identifier) {
		...
		for (DocumentBuilder builder : builders)
			for (Field field : builder.createDescriptorFields(image, identifier))
				doc.add(field);
		return doc;
	}
}
