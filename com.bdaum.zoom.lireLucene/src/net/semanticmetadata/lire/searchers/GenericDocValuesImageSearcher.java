...
/**
 * A generic image searcher for global features that uses DocValues instead of
 * Lucene text fields. Please make sure you created the index using th
 * useDocValues option in
 *
...
public class GenericDocValuesImageSearcher extends AbstractImageSearcher {
	...
	public GenericDocValuesImageSearcher(int maxHits, Class<? extends GlobalFeature> globalFeature,
			IndexReader reader) {
	...
	protected double findSimilar(LireFeature lireFeature) throws IOException {
		...
		// we read each and every document from the index and then we compare it to the
		// query.
		for (int i = 0; i < docs; i++) {
			...
			cachedInstance.setByteArrayRepresentation(binaryValue.bytes, binaryValue.offset, binaryValue.length);
			// cachedInstance.setByteArrayRepresentation(docValues.get(i).bytes,
			// docValues.get(i).offset, docValues.get(i).length);
	...
	// This is an approach based on DocValues. It's extremely fast, even without
	// caching, but I don't know if it's in
	...
	public ImageSearchHits search(int doc) throws IOException {
		...
		// BinaryDocValues binaryValues = MultiDocValues.getBinaryValues(reader,
		// lireFeature.getFieldName());
		if (docValues.advanceExact(doc)) {
			BytesRef binaryValue = docValues.binaryValue();
			lireFeature.setByteArrayRepresentation(binaryValue.bytes, binaryValue.offset, binaryValue.length);
		}
		// lireFeature.setByteArrayRepresentation(docValues.get(doc).bytes,
		// docValues.get(doc).offset, docValues.get(doc).length);
	...
	public ImageSearchHits search(Document doc, IndexReader reader) throws IOException {
		...
		TermQuery tq = new TermQuery(new Term(DocumentBuilder.FIELD_NAME_IDENTIFIER,
				doc.getValues(DocumentBuilder.FIELD_NAME_IDENTIFIER)[0]));
	...
	public ImageSearchHits search(BufferedImage image, IndexReader reader) throws IOException {
		...
		GlobalFeature globalFeature = globalDocumentBuilder.extractGlobalFeature(image,
