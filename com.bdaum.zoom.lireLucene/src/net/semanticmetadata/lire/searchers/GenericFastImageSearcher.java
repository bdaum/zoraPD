...
//                            localDocs.remove(localDocs.last());
    ...
    protected double getDistance(Document document, LireFeature lireFeature) {
    	IndexableField field = document.getField(fieldName); //bd
		if (field != null && field.binaryValue() != null && field.binaryValue().length > 0) {
//        if (document.getField(fieldName).binaryValue() != null && document.getField(fieldName).binaryValue().length > 0) {
            cachedInstance.setByteArrayRepresentation(field.binaryValue().bytes, field.binaryValue().offset, field.binaryValue().length);
...
//      if (doc.getField(fieldName).binaryValue() != null && doc.getField(fieldName).binaryValue().length > 0)
            lireFeature.setByteArrayRepresentation(field.binaryValue().bytes, field.binaryValue().offset, field.binaryValue().length);
        double maxDistance = findSimilar(reader, lireFeature);
