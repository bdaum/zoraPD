...
/**
 * This class allows for searching based on {@link MetricSpaces} HashingMode.
 * First a number of candidates is retrieved from the index, then the candidates
 * are re-ranked. The number of candidates can be tuned with the
 * numHashedResults parameter in the constructor. The higher this parameter, the
 * better the results, but the slower the search.
 *
...
public class MetricSpacesImageSearcher extends AbstractImageSearcher {
	...
	/**
	 * Creates a new searcher for MetricSpaces based indexed features.The field
	 * names are inferred from the entries in the reference point file.
	 *
	 * @param maximumHits
	 *            how many hits the searcher shall return.
	 * @param referencePointFile
	 *            the file created by MetricSpaces
	 * @see net.semanticmetadata.lire.indexers.hashing.MetricSpaces#indexReferencePoints(Class,
	 *      int, int, File, File)
	 */
	public MetricSpacesImageSearcher(int maximumHits, File referencePointFile)
	 ...
	 *            the file created by MetricSpaces
	 * @param useDocValues
	 *            if true, the docValues are used, rather that the Lucene fields.
	 * @param reader
	 *            needed to access the DocValues of the index. can be null if no
	 *            useDocValues==false
	 * @see net.semanticmetadata.lire.indexers.hashing.MetricSpaces#indexReferencePoints(Class,
	 *      int, int, File, File)
	 */
	public MetricSpacesImageSearcher(int maximumHits, File referencePointFile, boolean useDocValues, IndexReader reader)
			throws IllegalAccessException, InstantiationException, ClassNotFoundException {
		this.maximumHits = maximumHits;
	 ...
	 *            the data (ie. file) created by MetricSpaces
	 * @param numHashedResults
	 *            the number of candidate results retrieved from the index before
	 *            re-ranking.
	 * @see net.semanticmetadata.lire.indexers.hashing.MetricSpaces#indexReferencePoints(Class,
	 *      int, int, File, File)
	 */
	public MetricSpacesImageSearcher(int maximumHits, InputStream referencePoints, int numHashedResults)
			throws IllegalAccessException, InstantiationException, ClassNotFoundException {
		this.maximumHits = maximumHits;
	 ...
	 */
	public MetricSpacesImageSearcher(int maximumHits, InputStream referencePoints, int numHashedResults,
			boolean useDocValues, IndexReader reader)
			throws IllegalAccessException, InstantiationException, ClassNotFoundException {
		this.maximumHits = maximumHits;
		this.maxResultsHashBased = numHashedResults;
		this.useDocValues = useDocValues;
	...
	public ImageSearchHits search(Document doc, IndexReader reader) throws IOException {
		...
		if (useDocValues) {
			// find the id of the document in the reader, then do search ... TODO: find
			// another way instead of calling the searcher every time.
			TopDocs topDocs = searcher.search(new TermQuery(
					new Term(DocumentBuilder.FIELD_NAME_IDENTIFIER, doc.get(DocumentBuilder.FIELD_NAME_IDENTIFIER))),
					1);
			if (topDocs.totalHits > 0) {
				int docID = topDocs.scoreDocs[0].doc;
				if (docValues.advanceExact(docID)) {
					BytesRef binaryValue = docValues.binaryValue();
					queryFeature.setByteArrayRepresentation(binaryValue.bytes, binaryValue.offset, binaryValue.length);
					// queryFeature.setByteArrayRepresentation(docValues.get(docID).bytes,
					// docValues.get(docID).offset, docValues.get(docID).length);
					return search(MetricSpaces.generateBoostedQuery(queryFeature, numHashesUsedForQuery), queryFeature,
							searcher.getIndexReader());
				}
			}
		} else {
			queryFeature.setByteArrayRepresentation(doc.getBinaryValue(featureFieldName).bytes,
					doc.getBinaryValue(featureFieldName).offset, doc.getBinaryValue(featureFieldName).length);
	...
	private ImageSearchHits searchWithField(String hashes, GlobalFeature queryFeature, IndexReader reader)
			throws IOException {
	...
	private ImageSearchHits searchWithDocValues(String hashes, GlobalFeature queryFeature, IndexReader reader)
			throws IOException {
		// first search by text:
		IndexSearcher searcher = new IndexSearcher(reader);
		searcher.setSimilarity(new BaseSimilarity());
		...
		for (int i = 0; i < docs.scoreDocs.length; i++) {
			...
			// feature.setByteArrayRepresentation(docValues.get(docs.scoreDocs[i].doc).bytes,
			// docValues.get(docs.scoreDocs[i].doc).offset,
			// docValues.get(docs.scoreDocs[i].doc).length);
			tmpScore = queryFeature.getDistance(feature);
			assert (tmpScore >= 0);
			if (resultScoreDocs.size() < maximumHits) {
				resultScoreDocs.add(new SimpleResult(tmpScore, docs.scoreDocs[i].doc));
