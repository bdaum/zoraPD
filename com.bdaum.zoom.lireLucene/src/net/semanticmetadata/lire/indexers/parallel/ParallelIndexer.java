            ...
            while (!locallyEnded) {
                try {
                    ...
                    if (!locallyEnded) {   //&& tmp != null
                        ...
                        doc = allDocuments.get(tmp.getFileName()); // bd
                        String imageId = doc.get(DocumentBuilder.FIELD_NAME_IDENTIFIER); //bd
//                        fields = globalDocumentBuilder.createDescriptorFields(image);
                        fields = globalDocumentBuilder.createDescriptorFields(image, imageId); //bd
//                        doc = allDocuments.get(tmp.getFileName());
                        for (Field field : fields) {
                            doc.add(field);
                        }
...
//                        fields = localDocumentBuilder.createDescriptorFields(image);
                        fields = localDocumentBuilder.createDescriptorFields(image, imageId); //bd
                        for (Field field : fields) {
                            doc.add(field);
                        }
//                        fields = simpleDocumentBuilder.createDescriptorFields(image);
                        fields = simpleDocumentBuilder.createDescriptorFields(image, imageId); //bd
                        for (Field field : fields) {
                            doc.add(field);
                        }
