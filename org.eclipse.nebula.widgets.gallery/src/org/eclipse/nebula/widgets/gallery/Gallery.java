 ...
 *    Berthold Daum (selection tuning marked with bd,, marked with bd)
 *    Berthold Daum (allowed for count change during update(), marked with bd2)
 *    Berthold Daum (getOrderedSelection() marked with bd3)
*******************************************************************************/
 ...
 * @contributor Berthold Daum (selection tuning marked with bd, dynamic count
 *              changes marked with bd2, getOrderedSelection() marked with bd3)
 */
...
public class Gallery extends Canvas {
	...
	// private int[] selectionIndices = null; // bd
	private int[] selectionFlags = null; // bd
	...
	protected GalleryItem _getItem(GalleryItem parent, int index) {
		...
		if (index < parent.getItemCount()) {
			...
			if (parent.items == null) {
				return null;
				// } else { // bd2
			} else if (index < parent.items.length) { // bd2
	...
	public GalleryItem[] getOrderedSelection() { // bd3
		if (selection == null)
			return new GalleryItem[0];
		int k = 0;
		GalleryItem[] sortedSelection = new GalleryItem[selection.length];
		if (selectionFlags != null) {
			int g = 0;
			for (int i = 0; i < selectionFlags.length; i++) {
				int flags = selectionFlags[i];
				for (int j = 0; j < 32 && flags != 0; j++) {
					if ((flags & 1) != 0)
						sortedSelection[k++] = items[g + j];
					flags >>>= 1;
				}
				g += 32;
			}
		...
		for (GalleryItem galleryItem : items) {
			int[] itemFlags = galleryItem.selectionFlags;
			if (itemFlags != null) {
				GalleryItem[] subItems = galleryItem.items;
				int g = 0;
				for (int i = 0; i < itemFlags.length; i++) {
					int flags = itemFlags[i];
					for (int j = 0; j < 32 && flags != 0; j++) {
						if ((flags & 1) != 0)
							sortedSelection[k++] = subItems[g + j];
						flags >>>= 1;
					}
					g += 32;
				}
