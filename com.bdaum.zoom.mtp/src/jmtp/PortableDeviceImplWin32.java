...
class PortableDeviceImplWin32 implements PortableDevice {
    ...
    private String retrieveStringValue(PropertyKey key) throws COMException {
    	try {
    		...
    		if(e.getHresult() == COMException.E_POINTER) {
    			...
    			throw new DeviceClosedException("The device connection is closed."); //$NON-NLS-1$
    		}
			throw e;
    	}
    }
    ...
    public void open() throws IOException {
        try {
            openImpl(new PortableDeviceValuesImplWin32());
           	...
           	if(e.getHresult() == Win32WPDDefines.E_WPD_COULDNT_OPEN_DEVICE) { //bd
        		throw new IOException("Device connection lost.", e); //$NON-NLS-1$
        	}
        	if(e.getHresult() == Win32WPDDefines.E_WPD_DEVICE_ALREADY_OPENED) {
        		throw new DeviceAlreadyOpenedException("The device connection has already been opened."); //$NON-NLS-1$
        	}
			e.printStackTrace();
        }
    }
    ...
    public PortableDeviceObject[] getRootObjects() throws IOException {
        try {
        	...
        	if(e.getHresult() == COMException.E_POINTER || e.getHresult() == COMException.E_ENUMERATOR) { //bd
    			//there is no connection to the device
    			throw new DeviceClosedException("The device connection is closed."); //$NON-NLS-1$
    		}
			e.printStackTrace();
    ...
    public PortableDeviceObject[] getPortableDeviceObjectsFromPersistentUniqueIDs(
    		String[] persistentUniqueIDs) throws IOException {
    ...
    public PortableDeviceObject getPortableDeviceObjectsFromPersistentUniqueIDs(
    		String persistentUniqueID) throws IOException {
