...
public class ShapeRoi extends Roi {
	...
	* closest approximation of the original shape's curve segments with line segments.*/
	...
	private Shape roiToShape(Roi roi) {
		...
		switch(roi.getType()) {
			...
			case Roi.FREEROI: case Roi.TRACED_ROI:
				...
				xCoords = ((PolygonRoi)roi).getXCoordinates();
				yCoords = ((PolygonRoi)roi).getYCoordinates();
				shape = new GeneralPath(PathIterator.WIND_EVEN_ODD,nCoords); //Path2D.WIND_EVEN_ODD,nCoords); //bd
			...
			case Roi.POLYLINE: case Roi.FREELINE: case Roi.ANGLE:
				...
				xCoords = ((PolygonRoi)roi).getXCoordinates();
				yCoords = ((PolygonRoi)roi).getYCoordinates();
				shape = new GeneralPath(PathIterator.WIND_NON_ZERO,nCoords); //Path2D.WIND_NON_ZERO,nCoords); //bd
				((GeneralPath)shape).moveTo(xCoords[0], yCoords[0]);
				for (int i=1; i<nCoords; i++)
					((GeneralPath)shape).lineTo(xCoords[i],yCoords[i]);
				break;
	...
	Shape makeShapeFromArray(float[] array) {
		...
		Shape s = new GeneralPath(PathIterator.WIND_EVEN_ODD); //Path2D.WIND_EVEN_ODD); //bd
