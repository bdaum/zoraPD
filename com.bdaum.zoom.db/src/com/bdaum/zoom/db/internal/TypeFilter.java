...
public class TypeFilter extends AssetFilter implements ITypeFilter {
	...
	public boolean accept(Asset asset) {
		...
		case JPEG | RAW:
			return ImageConstants.JPEG.equals(format) || isRaw(format, mimeType);
		case JPEG | DNG:
			return ImageConstants.JPEG.equals(format) || isDng(format);
		case JPEG | DNG | RAW:
			return ImageConstants.JPEG.equals(format) || ImageConstants.IMAGE_X_RAW.equals(mimeType);
		case TIFF:
			return ImageConstants.TIFF.equals(format);

		case TIFF | RAW:
			return ImageConstants.TIFF.equals(format) || isRaw(format, mimeType);
		case TIFF | DNG:
			return ImageConstants.TIFF.equals(format) || isDng(format);
		case TIFF | DNG | RAW:
			return ImageConstants.TIFF.equals(format) || ImageConstants.IMAGE_X_RAW.equals(mimeType);
		case TIFF | JPEG:
			return ImageConstants.TIFF.equals(format) || ImageConstants.JPEG.equals(format);
		case TIFF | JPEG | RAW:
			return ImageConstants.JPEG.equals(format) || ImageConstants.TIFF.equals(format) || isRaw(format, mimeType);
		case TIFF | JPEG | DNG:
			return ImageConstants.TIFF.equals(format) || ImageConstants.JPEG.equals(format) || isDng(format);
		case TIFF | JPEG | DNG | RAW:
			return ImageConstants.TIFF.equals(format) || ImageConstants.JPEG.equals(format)
					|| ImageConstants.IMAGE_X_RAW.equals(mimeType);
		case OTHER:
			return !ImageConstants.TIFF.equals(format) && !ImageConstants.JPEG.equals(format)
					&& !ImageConstants.IMAGE_X_RAW.equals(mimeType) && !isMedia(format);
		case OTHER | RAW:
			return !ImageConstants.TIFF.equals(format) && !ImageConstants.JPEG.equals(format) && !isDng(format)
					&& !isMedia(format);
		case OTHER | DNG:
		...
		case OTHER | DNG | RAW:
			return !ImageConstants.TIFF.equals(format) && !ImageConstants.JPEG.equals(format) && !isMedia(format);
		case OTHER | JPEG:
			return !ImageConstants.TIFF.equals(format) && !ImageConstants.IMAGE_X_RAW.equals(mimeType)
					&& !isMedia(format);
		case OTHER | JPEG | RAW:
			return !ImageConstants.TIFF.equals(format) && !isDng(format) && !isMedia(format);
		case OTHER | JPEG | DNG:
			return !ImageConstants.TIFF.equals(format) && !isRaw(format, mimeType) && !isMedia(format);
		...
		case OTHER | TIFF:
			return !ImageConstants.JPEG.equals(format) && !ImageConstants.IMAGE_X_RAW.equals(mimeType)
					&& !isMedia(format);
		case OTHER | TIFF | RAW:
			return !ImageConstants.JPEG.equals(format) && !isDng(format) && !isMedia(format);
		case OTHER | TIFF | DNG:
			return !ImageConstants.JPEG.equals(format) && !isRaw(format, mimeType) && !isMedia(format);
		case OTHER | TIFF | DNG | RAW:
			return !ImageConstants.JPEG.equals(format) && !isMedia(format);
		case OTHER | TIFF | JPEG:
			return !ImageConstants.IMAGE_X_RAW.equals(mimeType) && !isDng(format) && !isMedia(format);
		...
		case MEDIA | RAW | DNG:
			return isMedia(format) || ImageConstants.IMAGE_X_RAW.equals(mimeType);
		case MEDIA | JPEG:
			return isMedia(format) || ImageConstants.JPEG.equals(format);
		case MEDIA | JPEG | RAW:
			return isMedia(format) || ImageConstants.JPEG.equals(format) || isRaw(format, mimeType);
		case MEDIA | JPEG | DNG:
			return isMedia(format) || ImageConstants.JPEG.equals(format) || isDng(format);
		case MEDIA | JPEG | DNG | RAW:
			return isMedia(format) || ImageConstants.JPEG.equals(format) || ImageConstants.IMAGE_X_RAW.equals(mimeType);
		case MEDIA | TIFF:
			return isMedia(format) || ImageConstants.TIFF.equals(format);
		case MEDIA | TIFF | RAW:
			return isMedia(format) || ImageConstants.TIFF.equals(format) || isRaw(format, mimeType);
		case MEDIA | TIFF | DNG:
			return isMedia(format) || ImageConstants.TIFF.equals(format) || isDng(format);
		case MEDIA | TIFF | DNG | RAW:
			return isMedia(format) || ImageConstants.TIFF.equals(format) || ImageConstants.IMAGE_X_RAW.equals(mimeType);
		case MEDIA | TIFF | JPEG:
			return isMedia(format) || ImageConstants.TIFF.equals(format) || ImageConstants.JPEG.equals(format);
		case MEDIA | TIFF | JPEG | RAW:
			return isMedia(format) || ImageConstants.JPEG.equals(format) || ImageConstants.TIFF.equals(format)
		...
		case MEDIA | OTHER | TIFF | JPEG:
			return !ImageConstants.IMAGE_X_RAW.equals(mimeType) && !isDng(format);
	...
	private static boolean isMedia(String format) {
		return CoreActivator.getDefault().getMediaSupport(format) != null;
	}

	private static boolean isDng(String format) {
		return ImageConstants.DNG_ADOBE_DIGITAL_NEGATIVE.equals(format) || ImageConstants.DNG.equals(format);
	}

	private static boolean isRaw(String format, String mimeType) {
		return ImageConstants.IMAGE_X_RAW.equals(mimeType) && !ImageConstants.DNG_ADOBE_DIGITAL_NEGATIVE.equals(format)
	...
	private Constraint getConstraint(DbManager dbManager, Query query, int formats) {
		...
		case RAW | DNG:
			return query.descend(QueryField.MIMETYPE.getKey()).constrain(ImageConstants.IMAGE_X_RAW);
		case JPEG:
		...
		case OTHER:
			...
			return query.descend(QueryField.MIMETYPE.getKey()).constrain(ImageConstants.IMAGE_X_RAW).not()
					.and(formatConstraint);
		case OTHER | RAW:
	...
	private static Constraint createMediaConstraint(Query query) {
		Constraint c1 = null;
		Set<String> mediaFormats = CoreActivator.getDefault().getMediaFormats();
		for (String format : mediaFormats) {
			Constraint c2 = query.descend(QueryField.FORMAT.getKey()).constrain(format);
	...
	private static Constraint createDngConstraint(Query query, boolean accept) {
		if (accept) {
			...
			return query.descend(QueryField.FORMAT.getKey()).constrain(ImageConstants.DNG).or(formatConstraint);
		}
		Constraint formatConstraint = query.descend(QueryField.FORMAT.getKey())
				.constrain(ImageConstants.DNG_ADOBE_DIGITAL_NEGATIVE).not();
		return query.descend(QueryField.FORMAT.getKey()).constrain(ImageConstants.DNG).not().and(formatConstraint);
	}

	private static Constraint createRawConstraint(Query query, boolean accept) {
		if (accept) {
			Constraint formatConstraint = query.descend(QueryField.MIMETYPE.getKey())
					.constrain(ImageConstants.IMAGE_X_RAW);
			formatConstraint = query.descend(QueryField.FORMAT.getKey())
					.constrain(ImageConstants.DNG_ADOBE_DIGITAL_NEGATIVE).not().and(formatConstraint);
			return query.descend(QueryField.FORMAT.getKey()).constrain(ImageConstants.DNG).not().and(formatConstraint);
		}
		Constraint formatConstraint = query.descend(QueryField.MIMETYPE.getKey()).constrain(ImageConstants.IMAGE_X_RAW)
				.not();
		formatConstraint = query.descend(QueryField.FORMAT.getKey())
				.constrain(ImageConstants.DNG_ADOBE_DIGITAL_NEGATIVE).or(formatConstraint);
		return query.descend(QueryField.FORMAT.getKey()).constrain(ImageConstants.DNG).or(formatConstraint);
	}
